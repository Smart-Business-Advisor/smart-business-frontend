...
const calculateFeasibility = (idea, assumptions) => {
  ...
  const paybackPeriod =
    annualReturnYear1 > 0 ? (cost / annualReturnYear1).toFixed(1) : "N/A";
    ...
    datasets: [
      {
        ...
        backgroundColor: "rgba(34, 110, 260, 0.8)", 
...
export default function FeasibilityAnalysisPage() {
  ...
  const [assumptions, setAssumptions] = useState(fixedAssumptions);
  const [results, setResults] = useState(null);

  const metricExplanations = {
    "Initial Investment": "The estimated capital needed to start the project.",
    "Expected ROI":
      "Expected annual return on your investment in the first year.",
    "Payback Period":
      "The estimated time needed to recover your initial investment.",
    "Contingency Budget":
      "The remaining budget available for operations or expansion.",
    "Risk Level":
      "Indicates the stability and predictability of the income stream.",
  ...
  useEffect(() => {
    ...
    const storedIdeasResult = localStorage.getItem("ideasResult"); 
    ...
    try {
      ...
      const userInputsData = storedIdeasResult ? JSON.parse(storedIdeasResult) : null;
      
      const realUserBudget = userInputsData?.input?.budget ? Number(userInputsData.input.budget) : 20000;

      const dynamicAssumptions = {
        ...fixedAssumptions,
        maxBudget: realUserBudget, 
      };
      
      setAssumptions(dynamicAssumptions); 
      ...
      const feasibilityResults = calculateFeasibility(ideaForCalculation, dynamicAssumptions);
  ...
  const npvStyles = {
    positive: {
      container: "bg-[#d1fae5] border-[#10b981] text-[#065f46]",
      value: "text-green-700",
      label: "Positive NPV — Likely Profitable",
    },
    negative: {
      container: "bg-[#fee2e2] border-[#ef4444] text-[#991b1b]",
      value: "text-red-700",
      label: "Negative NPV — High Risk",
    },
    zero: {
      container: "bg-white border-gray-300 text-gray-800",
      value: "text-gray-800",
      label: "Neutral NPV — Break-even",
    },
  };
  ...
  return (
    <div
      ref={reportRef}
      className="w-full min-h-screen p-10 bg-gray-50 overflow-auto"
    >
      <div className="max-w-7xl mx-auto flex flex-col gap-10">
        ...
        <div className="text-center bg-white p-6 rounded-xl shadow-xl border-b-4 border-blue-500">
          ...
          <p className="text-sm text-gray-500 mt-1">
           Investment: ${selectedIdea.cost.toLocaleString()} | Total Budget: ${assumptions.maxBudget.toLocaleString()}
          </p>
          ...
          {[
            {
              ...
              value: `$${selectedIdea.cost.toLocaleString()}`,
            },
            {
              label: "Expected ROI",
              value: `${(selectedIdea.profitRate * 100).toFixed(1)}%`,
            },
            {
            ...
            {
              label: "Risk Level",
              value: selectedIdea.risk,
              color:
                selectedIdea.risk === "Medium"
                  ? "text-yellow-600"
                  : "text-green-600",
            },
          ].map((metric) => (
            <Card
              key={metric.label}
              className="p-4 flex flex-col justify-center items-center text-center shadow-md"
            >
              <span
                className={`text-2xl font-extrabold ${metric.color || "text-blue-600"}`}
              >
                {metric.value}
              </span>
              ...
              <p className="text-xs text-gray-500 mt-1">
                {metricExplanations[metric.label]}
              </p>
            </Card>
          ))}
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  {Array.from(
                    { length: fixedAssumptions.projectionYears + 1 },
                    (_, i) => i,
                  ).map((year) => (
                    <th
                      key={year}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                ...
                <tr className="bg-gray-100/70">
                  ...
                  {results.cumulativeCF.map((cf, index) => (
                    <td
                      key={index}
                      className={`px-6 py-4 whitespace-nowrap text-right text-sm font-semibold ${cf < 0 ? "text-red-600" : "text-green-600"}`}
                    >
                      {cf < 0
                        ? `-$${Math.abs(cf).toLocaleString()}`
                        : `$${cf.toLocaleString()}`}
        ...
        {/* NPV */}
        <div
          className={`p-6 rounded-xl shadow-xl text-center border-l-4 ${npvClass}`}
        >
          ...
          <p
            className={`text-4xl font-extrabold mb-3 ${npvStyles[npvState].value}`}
          >
